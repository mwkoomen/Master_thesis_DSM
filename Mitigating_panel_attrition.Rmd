...
b <- ggplot(nbias, aes(x=Education,y=Value, color=Data)) + 
  ...
  #save
    png(file=paste0(wd,"/bias.png"),width=800, height=600, res=150)
    b
    dev.off() 
```

```{r bias-fig, out.width="75%", fig.cap="2017 Enrollment of 2016 school-leaving cohort."}
knitr::include_graphics(paste0(wd,"/bias.png"))
```
...
```{r qqplot, include=F}
png(file=paste0(wd,"/qq.png"),width=900, height=400, res=150)
par(mfrow=c(1,3))
...
knitr::include_graphics(paste0(wd,"/qq.png"))
```
...
load("test_vseq_ext.RData")
 ...
 p <- 
  ...
  #save
    png(file=paste0(wd,"/vseq3.png"),width=1200, height=800, res=150)
    p
    dev.off()
```

```{r vseq.fig, out.width="95%", fig.cap="Propensity mean-squared error per synthesis visit sequence."}
knitr::include_graphics(paste0(wd,"/vseq3.png"))
```
...
png(file=paste0(wd,"/synmeth2.png"),width=1400, height=1000, res=150)
u
dev.off()
```
...
knitr::include_graphics(paste0(wd,"/synmeth2.png"))
```

```{r joyreadtest, include=F}
...
png(file=paste0(wd,"/joyread_trans.png"),width=800, height=600, res=150)
...
png(file=paste0(wd,"/joyread.png"),width=800, height=600, res=150)
...
knitr::include_graphics(paste0(wd,"/joyread.png"))
```
...
```{r model.pick, include=F}
#Run find_model.R fist 
load("test_model_redux4.RData")
...
knitr::include_graphics(paste0(wd,"/model2.png"))
```
...
u <- multinom(us_enroll ~ .-ls_req-aspmf,
              data=d)
png(file=paste0(wd,"/resid.png"),width=900, height=900, res=150)
par(mfrow=c(2,2))
...
knitr::include_graphics(paste0(wd,"/resid.png"))
```
...
```{r loadsyn}
load(paste0(wd,"/syn2.Rdata"))
```

```{r compare.synth, include=F}
...
p <- ggplot(comp.data, aes(x=Value, y=Percent,fill=Data)) +
  ...
  theme(axis.text=element_text(size=5),
    ...
    png(file=paste0(wd,"/comp.png"),width=1200, height=900, res=150)
    p
    dev.off()
```

```{r comp.fig, out.width="100%", fig.cap="Marginal distributions for synthetic and observed data."}
knitr::include_graphics(paste0(wd,"/comp.png"))
```
...
```{r ci.overlap, message=F, include=F}
...
c <- ggplot(ci, aes(x=y,y=ci.overlap)) + 
  ...
  theme(axis.text=element_text(size=7),
        axis.title=element_text(size=8)) + 
  ylim(0.2,1)
    png(file=paste0(wd,"/cioverlap4.png"),width=1000, height=600, res=150)
    c
    dev.off()
```

```{r ci.fig, out.width="90%", fig.align='center', fig.cap="Box-plot for confidence interval overlaps per outcome variable"}
knitr::include_graphics(paste0(wd,"/cioverlap4.png"))
```
...
png(file=paste0(wd,"/tcap.png"),width=800, height=500, res=150)
b
dev.off() 
```

```{r tcfig,out.height="100%",fig.align="center",fig.cap="TCAP score per generated synthetic data set."}
knitr::include_graphics(paste0(wd,"/tcap.png"))
```
